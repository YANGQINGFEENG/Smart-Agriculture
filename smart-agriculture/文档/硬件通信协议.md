...
执行器上报时可携带feedback字段，用于存储设备特有状态信息：
...
```
用户操作 → 服务器下发控制指令 → 硬件执行 → 硬件双通道回执
    → 服务器更新状态 → 前端轮询检测 → 乐观更新UI → 异步同步
```
...
1. 用户点击控制按钮
      ↓
2. 前端发送POST /api/actuators/{id}/commands
      ↓
3. 服务器创建指令（pending状态）
      ↓
4. 服务器通过WebSocket推送指令到硬件
      ↓
5. 前端开始300ms间隔轮询（?frontend=true）
      ↓
6. 硬件执行命令
      ↓
7. 硬件通过WebSocket + HTTP双通道发送回执
      ↓
8. 服务器更新指令为executed，更新执行器状态
      ↓
9. 前端轮询检测到executed状态
      ↓
