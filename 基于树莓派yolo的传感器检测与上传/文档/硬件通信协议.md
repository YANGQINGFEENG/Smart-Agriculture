...
| gyro_available | boolean | MPU6050 陀螺仪硬件是否连接初始化成功 |
| gesture_control_enabled | boolean | 手势控制（陀螺仪云台控制）是否运行中 |
...
```
ws://服务器IP:8080?gateway_ip={网关IP地址}
```
...
| 参数 | 类型 | 必填 | 说明 |
|------|------|------|------|
| gateway_ip | string | 是 | 网关IP地址，用于服务器识别连接来源 |
| actuator_id | string | 否 | 单执行器连接时可选（网关模式使用gateway_register注册） |
| device_id | string | 否 | 设备唯一标识 |
| area | string | 否 | 区域订阅时必填 |

> **重要说明**：网关模式下，硬件端通过 `gateway_ip` 参数建立连接后，必须发送 `gateway_register` 消息注册所有执行器ID，服务器才能将命令正确推送到对应执行器。
...
#### 1. 网关连接与注册
```
硬件端 → WebSocket握手（携带gateway_ip参数） → 服务器
       ←-- welcome消息（连接成功确认） --
       → gateway_register（注册所有执行器ID） → 服务器
       → heartbeat（每30秒定时发送）
       ←-- heartbeat_ack（服务器响应） --
```

**gateway_register 消息格式**:
```json
{
...
> **关键**：服务器收到 gateway_register 后，将WebSocket连接与所有执行器ID关联，后续命令才能通过WebSocket实时推送到硬件端。
