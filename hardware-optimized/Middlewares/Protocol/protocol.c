...
/* ==================================== 静态变量 ==================================== */
static const char *g_device_id = DEVICE_ID;
                                        ...
                                        float soil_ph)
{
    ...
    memset(frame, 0, sizeof(SensorDataFrame));
    ...
    frame->soil_temperature = soil_temperature;
    frame->soil_ec = soil_ec;
    frame->soil_ph = soil_ph;
    
    /* 计算CRC（不包括header和footer） */
    uint16_t crc_len = sizeof(SensorDataFrame) - 3; /* 减去header, crc16, footer */
...
sys_error_t protocol_build_heartbeat_frame(HeartbeatFrame *frame)
{
    ...
    memset(frame, 0, sizeof(HeartbeatFrame));
    ...
    /* 计算CRC */
    uint16_t crc_len = sizeof(HeartbeatFrame) - 3;
    frame->crc16 = protocol_crc16((uint8_t*)&frame->msg_type, crc_len);
    frame->footer = PROTOCOL_FOOTER;
...
{
    if (frame == NULL || frame_size < 4) {
        return 0;
    }
    
    /* 获取CRC位置（倒数第3和第2个字节） */
    uint16_t received_crc = (frame[frame_size - 3] << 8) | frame[frame_size - 2];
    uint16_t calculated_crc = protocol_crc16(&frame[1], frame_size - 4);
...
{
    if (frame == NULL || out_buf == NULL || out_buf_size < frame_size) {
        return 0;
    }
