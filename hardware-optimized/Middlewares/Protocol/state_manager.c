...
ActuatorState* state_manager_get_actuator_state(const char *actuator_id)
{
    for (uint8_t i = 0; i < g_actuator_count; i++) {
        if (strcmp((const char*)g_actuator_states[i].actuator_id, actuator_id) == 0) {
...
sys_error_t state_manager_update_actuator_state(const char *actuator_id, uint8_t state)
{
    ...
    if (act_state) {
        ...
        if (g_actuator_count < MAX_ACTUATORS) {
            strncpy((char*)g_actuator_states[g_actuator_count].actuator_id, actuator_id, sizeof(g_actuator_states[g_actuator_count].actuator_id));
...
static void add_error_to_history(ErrorType error_type, const char *error_msg)
{
    if (g_error_count < MAX_ERROR_HISTORY) {
        ...
        strncpy(g_error_history[g_error_count].error_msg, error_msg, sizeof(g_error_history[g_error_count].error_msg));
        ...
        strncpy(g_error_history[MAX_ERROR_HISTORY - 1].error_msg, error_msg, sizeof(g_error_history[MAX_ERROR_HISTORY - 1].error_msg));
...
static void process_error(ErrorType error_type, const char *error_msg)
{
    debug_uart_printf("[Error] Type: %d, Message: %s\r\n", error_type, error_msg);
    
    /* 根据错误类型进行处理 */
    switch (error_type) {
        case ERROR_TYPE_HARDWARE:
            /* 硬件错误处理 */
            debug_uart_printf("[Error] Hardware error detected\r\n");
            break;
            
        case ERROR_TYPE_COMMUNICATION:
            /* 通信错误处理 */
            debug_uart_printf("[Error] Communication error detected\r\n");
            break;
            
        case ERROR_TYPE_COMMAND:
            /* 命令执行错误处理 */
            debug_uart_printf("[Error] Command execution error detected\r\n");
            break;
            
        case ERROR_TYPE_SYSTEM:
            /* 系统错误处理 */
            debug_uart_printf("[Error] System error detected\r\n");
            break;
...
static void check_system_state(void)
{
    ...
    if (g_state_mgr.system_state == SYS_STATE_ERROR) {
        ...
        if (sys_get_tick_diff(g_state_mgr.last_error_time, sys_get_tick()) > ERROR_RECOVERY_TIME) {
            g_state_mgr.system_state = SYS_STATE_NORMAL;
            debug_uart_printf("[System] System recovered to normal state\r\n");
        }
    }
}
